...
int main(int argc, char* argv[]) {
    ...
    if (argc > 1) {
        for (int i = 1; i < argc; i++) {
            if (std::string(argv[i]) == "--comments") {
                ...
                std::string file_name_argument = argv[i];
                ...
                std::cout << "Usage:\n" << argv[0] << "\n   --comments  (optional) it prints what every line of Brainfuck means\n   --filename:[your_file_name]  (optional) the output will not be printed on the terminal, instead it will be saved on [your_file_name]" << std::endl;
                return 0;
    ...
    cout << "\nOutput: " << convert_string_to_bf(text, file_name, comments) << "\n";
    return 0;
}